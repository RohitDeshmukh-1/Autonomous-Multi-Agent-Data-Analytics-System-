  ...
  return (
    <div className="flex flex-col h-full">
      <header className="px-5 py-3 border-b border-neutral-900 bg-[#09090b] flex-shrink-0">
        <h1 className="text-sm font-semibold text-neutral-100">Query History</h1>
        <p className="text-xs text-neutral-500 mt-0.5">Current session · {records.length} queries</p>
      </header>
        ...
        {records.map((rec) => (
          <div key={rec.id} className="card overflow-hidden">
            <button
              ...
              className="w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-neutral-900/50 transition-colors"
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm text-neutral-200 truncate">{rec.user_query}</p>
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <span className="text-xs text-neutral-600">{rec.created_at}</span>
                </div>
              </div>
              <button
                onClick={(e) => { e.stopPropagation(); handleDelete(rec.id) }}
                className="text-neutral-700 hover:text-red-400 transition-colors flex-shrink-0 mt-0.5"
              >
                <Trash2 size={14} />
              </button>
            </button>
            ...
            {expanded === rec.id && rec.insight_text && (
              <div className="px-4 pb-4 border-t border-neutral-900">
                <p className="text-sm text-neutral-300 mt-3 leading-relaxed">{rec.insight_text}</p>
              </div>
            )}
          </div>
        ))}
